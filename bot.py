...
YT_COOKIE_FILE = None
if YT_COOKIES_STR:
    # space-separated -> proper Netscape (tab-separated) me convert
    fixed_lines = []
    for raw in YT_COOKIES_STR.splitlines():
        line = raw.rstrip("\n\r")
        stripped = line.strip()
        if not stripped:
            fixed_lines.append("")
        ...
        if len(parts) >= 7:
            ...
            # Agar structure samajh nahi aaya to original line hi likh do
            fixed_lines.append(stripped)
    fixed_text = "\n".join(fixed_lines)
