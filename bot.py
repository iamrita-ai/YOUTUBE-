    ...
    try:
        ...
        if ("Sign in to confirm you’re not a bot" in msg) or \
           ("Sign in to confirm you're not a bot" in msg):
            raise Exception(
                "Ye YouTube video login / 'not a bot' protection ke saath hai.\n"
                "Is type ke videos ko bina browser login/cookies ke bot se download nahi kiya ja sakta."
            )
        raise Exception(msg)
...
async def start_cmd(client, m):
    ...
    await m.reply_text(
        ...
        "• Agar YouTube 'Sign in to confirm you’re not a bot' ya age check maange,\n"
        "  to aise videos ko bot se download nahi kar sakte."
    )
...
async def help_cmd(client, m):
    ...
    await m.reply_text(
        ...
        "⚠️ Jis video pe YouTube login / 'not a bot' / age‑check maangta hai,\n"
        "unhe bina browser cookies ke bot se download nahi kar sakte.\n"
        "Wo YouTube ki restriction hai."
    )
