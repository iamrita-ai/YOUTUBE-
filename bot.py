    ...
    try:
        ...
        if "does not look like a Netscape format cookies file" in msg:
            raise Exception(
                ...
                "Cookies ko sahi Netscape format me paste karo."
            )
...
async def start_cmd(client, m):
    ...
    await m.reply_text(
        ...
        "• Agar cookies galat/expire ho gayi to error me clearly likha aayega."
    )
...
async def help_cmd(client, m):
    ...
    await m.reply_text(
        ...
        "⚠️ Agar error aaye:\n"
        "• 'YT_COOKIES ka format galat hai' – to cookies sahi Netscape format me daalo.\n"
        "• 'Cookies set hone ke baad bhi YouTube not a bot maang raha hai' –\n"
